...
#loaded the teacher_evals data, and named it "data", changed the name to teacher_evals to be more specific
```
...
teacher_evals |>
  summarise(data)
#used glimpse() to show an overview of the teacher_eval data file
# For a brief overview I decided to use glimpse() in order to provide a clean short overview of the teacher_eval data.
```
...
# you may want to use code to answer this question
teacher_evals|>
  summarize(teacher_id,student_id)
# i am unsure what this section means or is asking for
```
...
teacher_evals |>
  rename(sex = gender) |>
  filter(resp_share*no_participants > 10) |>
  select(course_id : SET_score_avg, percent_failed_cur, academic_degree : sex)

cleaned_teacher_evals <- write_csv(teacher_evals, "data-clean/cleaned_teacher_eval.csv" )
  
#looked throuhg the r book as a double check, followed the prompt as best as i could struggled with turning ints into characters.
```
...
# code chunk for Q6
cleaned_teacher_evals |>
  filter(if_any(.cols = -course_id, is.na))|> 
...
# code chunk for Q7
cleaned_teacher_evals |>
  select(academic_degree,seniority,gender) |>
  distinct(academic_degree,seniority,gender, .keep_all = TRUE) |>
  count(academic_degree,seniority,gender) |>
  summarize(academic_degree,seniority,gender)
```
